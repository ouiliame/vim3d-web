...
GLint Scene::light_lookup[] = { GL_LIGHT0, GL_LIGHT1, GL_LIGHT2, GL_LIGHT3,
...
void
Scene::passiveMotion(int x, int y)
{
    if (ivMODE & EDIT_MODE) {
        Point a = window2scene(x, y);
        if (curve)
            curve->updateActivePoint(a.x, a.y, a.z);
    }
}

void
Scene::timerFunc(int value)
{
...
float
getAnim(vector<animation_t>& animations, int i, long unsigned int time)
{
    return animations[i].scalar *
             animations[i]
               .chan
               ->points[(int(time * animations[i].freqscalar + animations[i].timeShift)) %
                        animations[i].chan->points.size()]
               .y +
           animations[i].offset;
}
...
void
Scene::animCallbackFunc()
{
    float degs = 0.0f;
    if (animations.size() <= 0)
        return;
    for (unsigned int i = 0; i < animations.size(); i++) {
        if (!(animations[i].start <= time && time <= animations[i].end))
            continue;
        switch (animations[i].param) {
            case TX:
                if (isCamera(animOBJ))
                    C(animOBJ)->eye.x = anim(i);
                else
                    animOBJ->M.a03 = anim(i);
                break;
            case TY:
                if (isCamera(animOBJ))
                    C(animOBJ)->eye.y = anim(i);
                else
                    animOBJ->M.a13 = anim(i);
                break;
            case TZ:
                if (isCamera(animOBJ))
                    C(animOBJ)->eye.z = anim(i);
                else
                    animOBJ->M.a23 = anim(i);
                break;
            case RX:
                degs = diff(i);
                if (isCamera(animOBJ)) {
                    C(animOBJ)->tilt(degs);
                } else if (isObject(animOBJ)) {
                    Point a(1, 0, 0);
                    O(animOBJ)->rotateObj(degs, a);
                }
                break;
            case RY:
                degs = diff(i);
            ...
            case KD_R:
                if (isGeo(animOBJ))
                    G(animOBJ)->diffuse[0] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->diffuse[0] = clamp01(anim(i));
                break;
            case KD_G:
                if (isGeo(animOBJ))
                    G(animOBJ)->diffuse[1] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->diffuse[1] = clamp01(anim(i));
                break;
            case KD_B:
                if (isGeo(animOBJ))
                    G(animOBJ)->diffuse[2] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->diffuse[2] = clamp01(anim(i));
                break;
            case KD_A:
                if (isGeo(animOBJ))
                    G(animOBJ)->diffuse[3] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->diffuse[3] = clamp01(anim(i));
                break;
                /* specular */
            case KS_R:
                if (isGeo(animOBJ))
                    G(animOBJ)->specular[0] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->specular[0] = clamp01(anim(i));
                break;
            case KS_G:
                if (isGeo(animOBJ))
                    G(animOBJ)->specular[1] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->specular[1] = clamp01(anim(i));
                break;
            case KS_B:
                if (isGeo(animOBJ))
                    G(animOBJ)->specular[2] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->specular[0] = clamp01(anim(i));
                break;
            case KS_A:
                if (isGeo(animOBJ))
                    G(animOBJ)->specular[3] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->specular[3] = clamp01(anim(i));
                break;

                /* ambient */
            case KA_R:
                if (isGeo(animOBJ))
                    G(animOBJ)->ambient[0] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->ambient[0] = clamp01(anim(i));
                break;
            case KA_G:
                if (isGeo(animOBJ))
                    G(animOBJ)->ambient[1] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->ambient[1] = clamp01(anim(i));
                break;
            case KA_B:
                if (isGeo(animOBJ))
                    G(animOBJ)->ambient[2] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->ambient[2] = clamp01(anim(i));
                break;
            case KA_A:
                if (isGeo(animOBJ))
                    G(animOBJ)->ambient[3] = clamp01(anim(i));
                else if (isLight(animOBJ))
                    L(animOBJ)->ambient[3] = clamp01(anim(i));
                break;

                /* shininess */
            case KSHINE:
                if (isGeo(animOBJ))
                    G(animOBJ)->shininess[0] = anim(i);
                break;

                /*cloth*/
            case UPDATE:
                ((Cloth*)G(animations[i].object)->p)->updateDistribution(time);
                break;
        }
    }
}
...
float
getRefValue(vector<reference_t>& references, int i, int param, long unsigned int time)
{
    float val = 0.0f;
    switch (param) {
        case TX:
            if (isCamera(refOBJ))
                val = C(refOBJ)->eye.x;
            else if (isObject(refOBJ))
                val = O(refOBJ)->M.a03;
            else if (isPoly(refOBJ))
                val = PL(refOBJ)->vertices[time % PL(refOBJ)->vertices.size()].x;
            break;
        case TY:
            if (isCamera(refOBJ))
                val = C(refOBJ)->eye.y;
                ...
                val = PL(refOBJ)->vertices[time % PL(refOBJ)->vertices.size()].y;
            break;
        case TZ:
            if (isCamera(refOBJ))
                val = C(refOBJ)->eye.z;
                ...
                val = PL(refOBJ)->vertices[time % PL(refOBJ)->vertices.size()].z;
            break;
    }
    return references[i].scalar * val + references[i].offset;
}
#define ref(i)                                                                                     \
    getRefValue(references,                                                                        \
                i,                                                                                 \
                references[i].paramRef,                                                            \
                ((references[i].freqscalar * time) + references[i].timeShift))
#define tOBJ references[i].targObject
void
Scene::refCallbackFunc()
{
    if (references.size() <= 0)
        return;

    for (unsigned int i = 0; i < references.size(); i++) {
        if (!(references[i].start <= time && time <= references[i].end))
            continue;
        switch (references[i].paramTarget) {
            case TX:
                if (isCamera(tOBJ))
                    C(tOBJ)->eye.x = ref(i);
                else if (isObject(tOBJ))
                    O(tOBJ)->M.a03 = ref(i);
                break;
            case TY:
                if (isCamera(tOBJ))
            ...
            case KD_R:
                if (isGeo(tOBJ))
                    G(tOBJ)->diffuse[0] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->diffuse[0] = ref(i);
                break;
            case KD_G:
                if (isGeo(tOBJ))
                    G(tOBJ)->diffuse[1] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->diffuse[1] = ref(i);
                break;
            case KD_B:
                if (isGeo(tOBJ))
                    G(tOBJ)->diffuse[2] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->diffuse[2] = ref(i);
                break;
            case KD_A:
                if (isGeo(tOBJ))
                    G(tOBJ)->diffuse[3] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->diffuse[3] = ref(i);
                break;

            case KS_R:
                if (isGeo(tOBJ))
                    G(tOBJ)->specular[0] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->specular[0] = ref(i);
                break;
            case KS_G:
                if (isGeo(tOBJ))
                    G(tOBJ)->specular[1] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->specular[1] = ref(i);
                break;
            case KS_B:
                if (isGeo(tOBJ))
                    G(tOBJ)->specular[2] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->specular[2] = ref(i);
                break;
            case KS_A:
                if (isGeo(tOBJ))
                    G(tOBJ)->specular[3] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->specular[3] = ref(i);
                break;

            case KA_R:
                if (isGeo(tOBJ))
                    G(tOBJ)->ambient[0] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->ambient[0] = ref(i);
                break;
            case KA_G:
                if (isGeo(tOBJ))
                    G(tOBJ)->ambient[1] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->ambient[1] = ref(i);
                break;
            case KA_B:
                if (isGeo(tOBJ))
                    G(tOBJ)->ambient[2] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->ambient[2] = ref(i);
                break;
            case KA_A:
                if (isGeo(tOBJ))
                    G(tOBJ)->ambient[3] = ref(i);
                else if (isLight(tOBJ))
                    L(tOBJ)->ambient[3] = ref(i);
                break;

            case POS:
                if (isCamera(tOBJ) && isCamera(refOBJ)) {
            ...
            case LOOKAT:
                if (isLight(tOBJ) && isObject(refOBJ)) {
...
/* maybe use variable args (vargs) and also Generic so that you can do crazy refs between different
 * types of objects */
void
Scene::register_reference(Generic* ref,
                          int refParam,
                          Generic* targ,
                          int targParam,
                          float freqscalar,
...
void
Scene::register_animation(Object* obj,
                          int param,
                          Channel* chan,
                          float freqscalar,
                          float scalar,
...
showLight(Generic* o)
{
    if (isObject(o) && !O(o)->parent)
        O(o)->display(LIGHT_TYPE);
}
void
showGeometry(Generic* o)
{
    if (isObject(o) && !O(o)->parent)
        O(o)->display(GEOMETRY_TYPE);
}
void
Scene::displayObjects()
{
    for_each(allOBJS.begin(), allOBJS.end(), showGeometry);
}
void
Scene::displayLights()
{
    for_each(allOBJS.begin(), allOBJS.end(), showLight);
}

Generic*
Scene::getObjectByName(gen_t& list, String sname)
{
    gen_t::iterator cursor;
    for (cursor = list.begin(); cursor != list.end(); cursor++) {
        Generic* o = *cursor;
        String cmp = o->name;
        if (!cmp.compare(sname))
            return o;
        // if (o->objs.size() > 0) {
...
bool
Scene::nameExists(gen_t& list, String sname)
{
    gen_t::iterator cursor;
    for (cursor = list.begin(); cursor != list.end(); cursor++) {
        Generic* o = *cursor;
...
Generic*
Scene::getObjectByFile(String filepath)
{
...
screenWidth()
{
    return .3 * glutGet(GLUT_SCREEN_WIDTH);
}
inline int
screenHeight()
{
    return .5 * glutGet(GLUT_SCREEN_HEIGHT);
}
inline int
windowWidth()
{
...
Point
Scene::window2scene(int x, int y)
{
    GLint viewport[4];
    ...
    if (ivMODE & EDIT_MODE) {
        if (ivMODE & YAXIS_MODE)
            planeNormal = Point(1, 0, 0);
        if (curve)
            planePos = curve->lastActive;
    }
...
Scene::motion(int x, int y)
{
    ...
    if (btn_ == GLUT_LEFT_BUTTON && !toggleRMouse && !toggleMMouse) {
        ...
        if ((ivMODE & TRANSLATE_MODE) && camera) {
            if (isGeo(selected)) {
                Point camDir =
                  Point(camera->eye.x - camera->focus.x, 0, camera->eye.z - camera->focus.z);
...
void
Scene::mouse(int btn, int state, int x, int y)
{
...
void
Scene::keyboard(unsigned char key, int x, int y)
{
    ...
    if (key == 'i' || key == '1') {
        ...
        // ENTER RETURN
        if (ivMODE & COMMAND_MODE)
            Func::End_COMMAND_MODE();
        else if (ivMODE & EDIT_MODE)
            Func::End_EDIT_MODE();
        else if (ivMODE & INSERT_MODE)
            Func::End_INSERT_MODE();
        else
            Func::Begin_EDIT_MODE();
    }
    ...
    switch (key) {
        ...
        case 'h':
            ...
            window->current->scopeAmounty = 1.0f;
            break;
        case 'j':
            selected = PrevElement(allObjects, selected);
            Operations::List(emptyArgs);
            break;
        case 'k':
            selected = NextElement(allObjects, selected);
            Operations::List(emptyArgs);
            break;
        case 'l':
            /* cycle through draing modes */
            if (drawMODE & NO_INTERP) {
                drawMODE &= ~NO_INTERP;
                drawMODE |= BEZIER_INTERP;
            } else if (drawMODE & BEZIER_INTERP) {
                drawMODE &= ~BEZIER_INTERP;
                drawMODE |= BSPLINE_INTERP;
            } else if (drawMODE & BSPLINE_INTERP) {
                drawMODE &= ~BSPLINE_INTERP;
                drawMODE |= NO_INTERP;
            }
            break;
        ...
        case 'd':
            ...
            break;
        case 'o':
            curSTATE = (curSTATE & ORTHO_GRID) ? (curSTATE & ~ORTHO_GRID) : (curSTATE | ORTHO_GRID);
            break;
        case 'e':
            break;
        case 'c':
        ...
        case '9':
            char ss[512];
            sprintf(ss, "Capture%05lu.ppm", time);
            Render::CurrentScreen(String(ss), 0, 0, w, h);
            break;
        case '8':
            Func::Toggle_ORTHO_MODE();
            break;
        case '7':
            Func::Toggle_SHOW_AXIS();
            break;
        case '6':
            Func::Toggle_SHOW_TERM();
            break;
        case '5':
        ...
        case 't':
            ...
            break;
        case 's':
            Func::Toggle_SMOOTH_SD();
            break;
        case 'w':
            Func::Toggle_WIREFRAME();
            break;
        case 'p':
            Func::Toggle_POINT_DISP();
            break;
        case 'm':
            Func::Toggle_TRANSLATE_MODE();
            break;
        case 'r':
            Func::Toggle_ROTATE_MODE();
            break;
        case 'y':
            Func::Toggle_YAXIS_MODE();
            break;
        case 'u':
            Func::Undo();
            break;
        ...
        case '-':
            ...
            break;
        case '`':
            camera->up = Point(0, 1, 0);
            break;
        case 'z':
            toggleRMouse = !toggleRMouse;
            if (toggleRMouse)
                toggleMMouse = false;
            break;
        case 'x':
            toggleMMouse = !toggleMMouse;
            if (toggleMMouse)
                toggleRMouse = false;
            break;
    }
...
void
Scene::reshape(int width, int height)
{
    w = width;
    h = height;
...
void
Scene::set2DMode(float x, float y, float w, float h)
{
...
Scene::showScene()
{
    if (curSTATE & ORTHO_GRID)
        Scene::constructionPlane();
    if (curSTATE & SHOW_AXIS)
        Scene::axis();
    Scene::printCurves();
    Scene::displayObjects();
...
Scene::addGeo(Geo* g)
{
    allObjects.push_back(GEN(g));
    setActive(GEN(g));
}
void
Scene::addPoly(Poly* p)
{
    allObjects.push_back(GEN(p));
    setActive(GEN(p));
}
void
Scene::addTexture(Texture* t)
{
    allObjects.push_back(GEN(t));
}
void
Scene::addLight(Light* l)
{
    allObjects.push_back(GEN(l));
    setActive(GEN(l));
}
void
Scene::addCamera(Camera* c)
{
    allObjects.push_back(GEN(c));
    setActive(GEN(c));
}
void
Scene::addChannel(Channel* c)
{
    allObjects.push_back(GEN(c));
    setActive(GEN(c));
}
...
void
Scene::dumpScene(String filename)
{
    ...
    if (references.size() > 0) {
        ...
        for (; csr != references.end(); csr++) {
            reference_t a = *csr;
            if (!a.refObject || !a.targObject)
                continue;
            if (isGeo(a.targObject) && G(a.targObject)->inst)
                continue;
            fprintf(fd,
                    "reference %s %s %s %s %d %f %f %d %lu %lu\n",
