...
help_t help[] = {
    ...
    { OP_REGISTER,
      "register <object> <param> <channel> [<freqScalar> <scalar> <offset> <timeShift> <start> "
      "<end>]",
      "register an animation",
      false },
    { OP_REFERENCE,
      "reference <refObject> <refParam> <object> <param> [<freqScalar> <scalar> <offset> "
      "<timeShift> <start> <end>]",
      "allows objects parameters to be controlled by others",
      false },
    { OP_SWEEP,
      "sweep <crossSection> [<hallway>, <onpath>]",
      "sweeps an object along a curve, onpath->along selected positions",
      true },
    { OP_PATH,
...
bool
Operations::add(String cmdName)
{
    OP_TYPE type = (OP_TYPE)NULL;
    for (unsigned int i = 0; i < sizeof(Map) / sizeof(cmdMap_t); i++) {
        if (!strcmp(cmdName.c_str(), Map[i].name))
            type = Map[i].type;
    }
    for (unsigned int i = 0; i < MAX_OPS; i++) {
        if (type == help[i].type)
            return help[i].append;
    }
    return false;
}

String
wrong_args(OP_TYPE type)
{
    return String(WRONG_ARGUMENTS + String("\n") + String(help[type].help));
}

void
threeFloats(vector<String>& args, float* x, float* y, float* z)
{
    if (args.size() >= 3) {
        *x = s2f(0);
        *y = s2f(1);
...
void
fourFloats(vector<String>& args, float* x, float* y, float* z, float* w)
{
    if (args.size() >= 4) {
...
/* closes the window */
void
Operations::quit(vector<String> args)
{
    Scene::window->close();
}
...
void
Operations::save(vector<String> args)
{
    if (args.size() > 0) {
        ...
        outbuf_ = String("Please specify a Scene name");
}
...
void
Operations::readOBJ(vector<String> args)
{
    ...
    if ((p = PL(Scene::getObjectByFile(filename))) == NULL) {
        ...
        outbuf_ = String(filename + " already in memory");
    P(g) = p;
...
void
Operations::write(vector<String> args)
{
    if (args.size() > 0) {
...
void
Operations::zNear(vector<String> args)
{
    if (args.size() > 0) {
        SceneCAM->zNear = s2f(0);
        outbuf_ = String("zNear set");
    } else
        outbuf_ = String(NO_AMOUNT_SPECIFIED);
}
...
void
Operations::zFar(vector<String> args)
{
    if (args.size() > 0) {
        SceneCAM->zFar = s2f(0);
...
void
Operations::invertNormals(vector<String> args)
{
    if (isGeo(selOBJ)) {
        if (P(selOBJ)) {
...
void
Operations::fovy(vector<String> args)
{
    if (args.size() > 0) {
        SceneCAM->fovy = s2f(0);
...
void
Operations::draw(vector<String> args)
{
}
...
void
Operations::normalMap(vector<String> args)
{
    if (args.size() > 0) {
        String filename = args[0];
        Image* normmap = NULL;
        ...
        if (isGeo(selOBJ)) {
            ...
            if (!P(selOBJ)) {
                for (unsigned int o = 0; o < G(selOBJ)->objs.size(); o++) {
                    if (isGeo(G(selOBJ)->objs[o]))
                        G(G(selOBJ)->objs[o])->normalMap = normmap;
                }
            }
            outbuf_ = String("Map applied");
        } else
            outbuf_ = String("texture in memory, but not applied to anything");
...
void
Operations::texture(vector<String> args)
{
    if (args.size() > 0) {
        String filename = args[0];
        Generic* obj = selOBJ;
...
void
Operations::Bezier(vector<String> args)
{
    String lod = String();
    if (args.size() > 0)
        lod = args(0);
...
void
Operations::Bspline(vector<String> args)
{
    String lod = String();
    if (args.size() > 0)
        lod = args(0);
...
void
Operations::Bezier2(vector<String> args)
{
    String lod = String();
    if (args.size() > 0)
        lod = args(0);
...
void
Operations::Extrude(vector<String> args)
{
    if (args.size() == 3) {
        if (isGeo(selOBJ)) {
...
void
Operations::Revolve(vector<String> args)
{
    if (args.size() == 4) {
        if (isGeo(selOBJ)) {
            float degs, rx, ry, rz;
...
void
Operations::compute(vector<String> args)
{
    if (isGeo(selOBJ) && P(selOBJ)) {
        if (!args.size()) {
...
void
Operations::bounds(vector<String> args)
{
    if (isGeo(selOBJ)) {
...
void
Operations::diffuse(vector<String> args)
{
    if (args.size() == 4) {
        float r, g, b, a;
...
void
Operations::specular(vector<String> args)
{
    if (args.size() == 4) {
        float r, g, b, a;
        fourFloats(args, &r, &g, &b, &a);
        ...
        outbuf_ = wrong_args(OP_SPECULAR);
}
...
void
Operations::ambient(vector<String> args)
{
    if (args.size() == 4) {
        float r, g, b, a;
        fourFloats(args, &r, &g, &b, &a);
...
void
Operations::shininess(vector<String> args)
{
    if (args.size() == 1) {
        if (isGeo(selOBJ)) {
        ...
        outbuf_ = wrong_args(OP_SHININESS);
}

void
Operations::reflection(vector<String> args)
{
    if (args.size() == 3) {
        if (isGeo(selOBJ)) {
        ...
        outbuf_ = wrong_args(OP_REFLECTION);
}
void
Operations::refraction(vector<String> args)
{
    if (args.size() == 1) {
        if (isGeo(selOBJ)) {
        ...
        outbuf_ = wrong_args(OP_REFRACTION);
}

void
Operations::emission(vector<String> args)
{
    if (args.size() == 4) {
        if (isGeo(selOBJ)) {
        ...
        outbuf_ = wrong_args(OP_EMISSION);
}

void
Operations::rotate(vector<String> args)
{
    if (args.size() == 4) {
        float deg, x, y, z;
        ...
        outbuf_ = wrong_args(OP_ROTATE);
}

void
Operations::translate(vector<String> args)
{
    if (args.size() == 3) {
        float tx, ty, tz;
        ...
        outbuf_ = wrong_args(OP_TRANSLATE);
}

void
Operations::scale(vector<String> args)
{
    if (args.size() == 3) {
        if (isGeo(selOBJ)) {
            float sx, sy, sz;
            ...
            if (sx && sy && sz) {
                ...
                outbuf_ = String("cannot scale by 0");
        } else
            outbuf_ = String(NO_GEO_SELECTED);
    } else
        outbuf_ = wrong_args(OP_SCALE);
}

void
Operations::add(vector<String> args)
{
    if (args.size() >= 1) {
        String type = args(0);
        if (match("light")) {
            ...
            if (numLights < GL_MAX_LIGHTS) {
                ...
                outbuf_ = String("you have the maximum number of lights");
            ...
            if (args.size() == 1) {
                ...
                // bscenes wont work after this ... Geo * grid = (Geo *) new
                // Grid((s2i(1)>2)?s2i(1):2);
            ...
            Geo* ring1 =
              (Geo*)new Ellipse(height - halfringWidth, width - halfringWidth, Vdepth, false);
            Geo* ring2 =
              (Geo*)new Ellipse(height + halfringWidth, width + halfringWidth, Vdepth, false);
            Geo* ring = new Geo();
            ...
            Point p1, p2, p3, p4;
            while (true) {
                if (cursor2 >= outerRing->vertices.end() - 1)
                    break; // less than 3 vertices on this side
                p1 = *(cursor);
                p2 = *(cursor2);
                cursor2++;
                p3 = *(cursor2);
...
void
Operations::load(vector<String> args)
{
    if (args.size() == 1) {
        ...
        outbuf_ = wrong_args(OP_LOAD);
}

void
Operations::Delete(vector<String> args)
{
...
/* window functions */
void
Operations::Vexplore(vector<String> args)
{
    Scene::window->splitVertical();
}
void
Operations::Sexplore(vector<String> args)
{
    Scene::window->splitHorizontal();
}

void
Operations::Matrix(vector<String> args)
{
    if (args.size() == 16) {
        if (selOBJ != NULL) {
            String arg1 = args(0);
            ...
            String arg9 = args(8);
            String arg10 = args(9);
            String arg11 = args(10);
            String arg12 = args(11);
            String arg13 = args(12);
            String arg14 = args(13);
            String arg15 = args(14);
            String arg16 = args(15);

            O(selOBJ)->M.a00 = atof(arg1.c_str());
            O(selOBJ)->M.a01 = atof(arg2.c_str());
            O(selOBJ)->M.a02 = atof(arg3.c_str());
            O(selOBJ)->M.a03 = atof(arg4.c_str());
            O(selOBJ)->M.a10 = atof(arg5.c_str());
            O(selOBJ)->M.a11 = atof(arg6.c_str());
            O(selOBJ)->M.a12 = atof(arg7.c_str());
            O(selOBJ)->M.a13 = atof(arg8.c_str());
            O(selOBJ)->M.a20 = atof(arg9.c_str());
            O(selOBJ)->M.a21 = atof(arg10.c_str());
            O(selOBJ)->M.a22 = atof(arg11.c_str());
            O(selOBJ)->M.a23 = atof(arg12.c_str());
            O(selOBJ)->M.a30 = atof(arg13.c_str());
            O(selOBJ)->M.a31 = atof(arg14.c_str());
            O(selOBJ)->M.a32 = atof(arg15.c_str());
            O(selOBJ)->M.a33 = atof(arg16.c_str());
        }
    } else
        outbuf_ = wrong_args(OP_MATRIX);
}

void
Operations::Position(vector<String> args)
{
    if (args.size() == 3) {
        float x, y, z;
        threeFloats(args, &x, &y, &z);
        ...
        outbuf_ = wrong_args(OP_POSITION);
}

void
Operations::Up(vector<String> args)
{
    if (args.size() == 3) {
        if (isCamera(selOBJ)) {
            float x, y, z;
        ...
        outbuf_ = wrong_args(OP_UP);
}

// lookat <obj> <lookatObject> [<start> <end>]
void
Operations::Lookat(vector<String> args)
{
    if (args.size() >= 2) {
        ...
        outbuf_ = wrong_args(OP_LOOKAT);
}

void
Operations::Children(vector<String> args)
{
    if (args.size() >= 2) {
        String type = args[0];
        if (match("lookat")) {
            ...
            if (isObject(selOBJ) && lookatObj) {
                for (unsigned int i = 0; i < OBJS(selOBJ).size(); i++) {
                    Scene::register_reference(
                      lookatObj, POS, GEN(OBJS(selOBJ)[i]), LOOKAT, 1, -1, 0, 0, start, end);
                }
            }
        }
    }
...
void
Operations::Name(vector<String> args)
{
    if (args.size() == 1) {
...
void
Operations::Clear(vector<String> args)
{
    outbuf_.clear();
}

void
Operations::clearall(vector<String> args)
{
    if (selOBJ) {
        if (isGeo(selOBJ)) {
            if (P(selOBJ))
...
void
Operations::Properties(vector<String> args)
{
    if (isObject(selOBJ)) {
        outbuf_ = O(selOBJ)->propertyInfo();
    } else if (isChannel(selOBJ))
        outbuf_ = CH(selOBJ)->propertyInfo();
}

void
Operations::MatrixInfo(vector<String> args)
{
    if (isObject(selOBJ)) {
...
void
Operations::History(vector<String> args)
{
    if (isObject(selOBJ)) {
...
void
printLight(Generic* o)
{
    if (isLight(o))
        outbuf_.append(o->printInfo());
}
void
printPolyInfo(Generic* p)
{
    if (isPoly(p))
        outbuf_.append(p->printInfo());
}
void
printTextureInfo(Generic* t)
{
    if (isTexture(t))
        outbuf_.append(t->printInfo());
}
void
printChannelInfo(Generic* c)
{
    if (isChannel(c))
        outbuf_.append(c->printInfo());
}

void
Operations::List(vector<String> args)
{
    outbuf_.clear();
    if (args.size() == 0) {
...
void
Operations::View(vector<String> args)
{
    if (args.size() == 1) {
        String type = args[0];
        if (match("model"))
            Scene::window->changeView(MODEL_VIEW);
        else if (match("channel"))
            Scene::window->changeView(CHANNEL_VIEW);
        else if (match("pnormals"))
            Func::Toggle_POINT_NORMALS();
        else if (match("pnums"))
            Func::Toggle_POINT_NUMS();
        else if (match("fnormals") || match("normals"))
            Func::Toggle_FACE_NORMALS();
        else if (match("fnums") || match("nums"))
            Func::Toggle_FACE_NUMS();
    } else if (args.size() == 2) {
        String type = args[0];
        ...
        outbuf_ = wrong_args(OP_VIEW);
}

// can we use eval??
void
Operations::ChannelOp(vector<String> args)
{
    if (args.size() == 1 && isChannel(selOBJ)) {
        ...
        if (match("add")) {
            ...
            *storeChan = CH(op1)->project(CH(op2));
            ...
            if (args.size() == 5) {
                ...
                if (isChannel(op3))
                    *storeChan = Channel::combine(CH(op1), CH(op2), CH(op3));
            } else
                *storeChan = Channel::combine(CH(op1), CH(op2));
        ...
        outbuf_ = wrong_args(OP_CHANNEL);
}

void
Operations::LightOp(vector<String> args)
{
    if (isLight(selOBJ)) {
        if (args.size() >= 2) {
            String type = args(0);
            ...
            outbuf_ = wrong_args(OP_LIGHT);
    } else
        outbuf_ = String("no light selected");
}

void
Operations::ObjectOp(vector<String> args)
{
    if (args.size() >= 2) {
        String type = args(0);
        if (match("group")) {
            ...
            if (match("child")) {
                ...
                if (isObject(p) && isObject(c)) {
                    ...
                    outbuf_ = String("specified objects don't exist");
            }
        } else
            outbuf_ = wrong_args(OP_OBJECT);
    } else
        outbuf_ = wrong_args(OP_OBJECT);
}

void
Operations::GeoOp(vector<String> args)
{
    if (isGeo(selOBJ)) {
        if (args.size() >= 2) {
            String type = args(0);
        ...
        outbuf_ = String("no geo selected");
}

void
Operations::CameraOp(vector<String> args)
{
    if (isCamera(SceneCAM)) {
        if (args.size() >= 1) {
            ...
            outbuf_ = wrong_args(OP_CAMERA);
    } else
        outbuf_ = String("no camera selected");
}

/* copy <target> <template> */

void
Operations::Copy(vector<String> args)
{
    if (args.size() >= 2) {
        ...
        outbuf_ = wrong_args(OP_COPY);
}

// help

void
Operations::Help(vector<String> args)
{
    int i = 0;
    outbuf_.clear();
    for (; i < MAX_OPS; i++) {
...
void
Operations::Select(vector<String> args)
{
    if (args.size() == 1) {
        Generic* o = objByName(0);
        if (o)
        ...
        outbuf_ = wrong_args(OP_SELECT);
}

void
Operations::BeginOp(vector<String> args)
{
    ivMODE = BEGIN_MODE;
    if (args.size() == 1) {
        String type = args(0);
        ...
        outbuf_ = wrong_args(OP_BEGIN);
}

void
Operations::EndOp(vector<String> args)
{
...
// <end>]
void
Operations::Register(vector<String> args)
{
    if (args.size() >= 3) {
        Generic* o = objByName(0);
        if (!isObject(o)) {
        ...
        if (c)
            Scene::register_animation(
              O(o), param, c, freqscalar, scalar, offset, timeShift, start, end);

    } else
        outbuf_ = wrong_args(OP_REGISTER);
}

// reference <object> <param> <object> <param> [<freqScalar> <scalar> <offset> <timeShift>
// <start> <end>]
void
Operations::Reference(vector<String> args)
{
    if (args.size() >= 4) {
        ...
        Scene::register_reference(
          ref, paramRef, targ, paramTarg, freqscalar, scalar, offset, timeShift, start, end);
        outbuf_ = String("registered reference");
    } else
        outbuf_ = wrong_args(OP_REFERENCE);
}

void
Operations::Sweep(vector<String> args)
{
    if (args.size() == 1) {
        if (isGeo(selOBJ)) {
...
// path <target> <path> [<dx> <dy> <dz> <freqscalar> <timeShift> <start> <end>]
void
Operations::Path(vector<String> args)
{
    if (args.size() >= 2) {
        Generic* target = objByName(0);
        ...
        if (isGeo(path))
            ...
            outbuf_ = String("the path object must be or must contain a polygon");
            return;
        }
        ...
        Scene::register_reference(
          GEN(p), TX, target, TX, freqscalar, 1.0f, dx, timeShift, start, end);
        Scene::register_reference(
          GEN(p), TY, target, TY, freqscalar, 1.0f, dy, timeShift, start, end);
        Scene::register_reference(
          GEN(p), TZ, target, TZ, freqscalar, 1.0f, dz, timeShift, start, end);
    } else
        outbuf_ = wrong_args(OP_PATH);
}

// poly <centoid|texture> [<s> <t>]
void
Operations::PolyOp(vector<String> args)
{
    if (args.size() >= 1) {
        Poly* p = NULL;
        if (isPoly(selOBJ))
            p = PL(selOBJ);
        else if (isGeo(selOBJ) && P(selOBJ))
            p = P(selOBJ);
        else {
            outbuf_ = String(NO_GEO_SELECTED);
            return;
        ...
        outbuf_ = wrong_args(OP_POLY);
}

void
Operations::cloth(vector<String> args)
{
    Geo* cloth = new Geo();
    cloth->p = new Cloth();
    if (args.size() > 0) {
...
void
Operations::tree(vector<String> args)
{
    /*
       if(args.size()>=2){
       Geo * tree = new Geo();
...
void
Operations::RayTrace(vector<String> args)
{
    printf("raytracing...\n");
    if (args.size() >= 1) {
        if (args.size() == 1)
...
void
Operations::Render(vector<String> args)
{
    if (args.size() >= 1) {
        if (args.size() == 1)
            Render::CurrentScreen(args[0], 0, 0, Scene::w, Scene::h);
        else if (args.size() >= 2) {
            String type = args[0];
            if (match("set") || match("display")) {
                bool render = (bool)s2i(1);
                if (render)
                    Scene::curSTATE |= RENDERING;
                else
                    Scene::curSTATE &= ~RENDERING;
            } else if (match("raytrace")) {
                bool render = (bool)s2i(1);
                if (render)
                    Scene::curSTATE |= RAYTRACING;
                else
                    Scene::curSTATE &= ~RAYTRACING;
                if (render)
                    Scene::time = Render::start;
...
void
Operations::Timeline(vector<String> args)
{
    if (args.size() >= 1) {
        String type = args[0];
        if (match("play")) {
        ...
        outbuf_ = wrong_args(OP_TIMELINE);
    printf("time: %lu\n", Scene::time);
}

void
Operations::Display(vector<String> args)
{
    if (args.size() >= 1) {
        String type = args[0];
        if (match("none"))
            curSTATE = DISPLAY_STATE;
        else if (match("default"))
            curSTATE = DEFAULT_STATE;
        else if (match("wireframe"))
            Func::Toggle_WIREFRAME();
        else if (match("axis"))
            Func::Toggle_SHOW_AXIS();
        else if (match("ortho"))
            Func::Toggle_ORTHO_GRID();
        else if (match("texture"))
            Func::Toggle_TEXTURE_ON();
        else if (match("lights"))
            Func::Toggle_LIGHTS_ON();
        else if (match("displaylists"))
            Func::Toggle_DISPLAY_LIST();
        else if (match("select"))
            Func::Toggle_SHOW_SELECT();
    } else if (args.size() == 2) {
        String type = args[0];
        if (match("gridsize"))
            Scene::gridSize = s2i(1);
        else if (match("gridstep"))
            Scene::gridStep = s2i(1);
    }
}

void
Operations::Convert(vector<String> args)
{
    if (args.size() == 2) {
...
void
Operations::Eval(vector<String> args)
{
...
void
Operations::server(vector<String> args)
{
    if (args.size() > 0) {
        String type = args[0];
        if (match("start")) {
...
void
Operations::connect(vector<String> args)
{
    if (args.size() > 0) {
        if (args.size() >= 2) {
            if (!Scene::client)
                Scene::client = new Client();
...
void
Operations::send(vector<String> args)
{
...
void
Operations::testing(vector<String> args)
{
}
Operations::~Operations() {}
